...
function DashboardPage() {
    ...
    const { overview, isLoading: marketLoading, lastUpdated } = useMarketData({
        symbol: 'BTC-PERP',
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <StatCard
                    label="BTC Price"
                    value={marketLoading ? '...' : `$${overview?.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                ...
                <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl p-6">
                    ...
                    <div className="mt-2 flex gap-1">
                        {Array.from({ length: 10 }).map((_, i) => (
                            <div
                                key={i}
                                className={`h-4 flex-1 rounded ${i < (overview?.volatility24h ?? 0) / 10 * 10
                                        ? 'bg-warning'
                                        : 'bg-[hsl(var(--muted))]'
                                    }`}
                            />
                ...
                <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl p-6">
                    ...
                    <p className="text-2xl font-bold">
                        {overview?.nextFundingTime
                            ? new Date(overview.nextFundingTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                            : '...'
                        }
                    </p>
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {topTraders?.map((trader) => (
                                <tr key={trader.rank} className="border-b border-[hsl(var(--border))] last:border-0 hover:bg-[hsl(var(--accent))] transition-colors">
