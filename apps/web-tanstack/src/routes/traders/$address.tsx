...
import { z } from 'zod'
import { VirtualTradeList, generateMockTrades } from '../../components/tables/VirtualTradeList'
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <StatCard label="24h PnL" value={`$${trader?.pnl24h.toLocaleString()}`} positive={(trader?.pnl24h ?? 0) >= 0} />
                <StatCard label="7d PnL" value={`$${trader?.pnl7d.toLocaleString()}`} positive={(trader?.pnl7d ?? 0) >= 0} />
                <StatCard label="Win Rate" value={`${trader?.winRate}%`} />
                <StatCard label="Sharpe Ratio" value={`${trader?.sharpeRatio}`} positive={(trader?.sharpeRatio ?? 0) >= 0} />
                ...
                <StatCard label="30d PnL" value={`$${trader?.pnl30d.toLocaleString()}`} positive={(trader?.pnl30d ?? 0) >= 0} />
            </div>
                    ...
                    <tbody>
                        {trader?.positions.map((pos, i) => (
                            <tr key={i} className="border-b border-[hsl(var(--border))] last:border-0">
                                ...
                                <td className={`py-3 text-right font-medium ${pos.pnl >= 0 ? 'text-success' : 'text-destructive'}`}>
