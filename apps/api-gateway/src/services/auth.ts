...
export class AuthService {
  ...
  async verifyAccessToken(token: string): Promise<UserPayload> {
    try {
      ...
      this.logger.warn('Access token verification failed', { error: error instanceof Error ? error.message : String(error) });
  ...
  async refreshAccessToken(refreshToken: string): Promise<AuthTokens> {
    try {
      ...
      this.logger.warn('Refresh token verification failed', { error: error instanceof Error ? error.message : String(error) });
