...
export const marketApi = {
  ...
  getOHLCV: async (symbol: string, timeframe: string, limit = 100): Promise<OHLCV[]> => {
    const response = await apiClient.post<{ result: { data: OHLCV[] } }>('/api/trpc/market.ohlcv', {
      symbol,
      timeframe,
      limit,
    });
...
export const authApi = {
  ...
  login: async (
    ...
    const response = await apiClient.post<{
      result: { data: { token: string; refreshToken: string } };
    }>('/api/trpc/auth.walletLogin', { walletAddress, signature, message });
    return response.result.data;
  },
